...
function token(prefix, id){
  const value = `${prefix}-${id}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  return value;
}
...
function vitals(id){
  const seed = [...String(id || 'p1')].reduce((a, c) => a + c.charCodeAt(0), 0);
  const make = (base, spread, n = 30) => Array.from({length:n}, (_, i) => ({
    t: new Date(Date.now() - (n - i) * 30000).toISOString(),
    v: Math.round(base + Math.sin((i + seed) / 3) * spread + ((i + seed) % 5 - 2))
  }));
  return {
    HR: make(82 + seed % 18, 9),
    SPO2: make(95 - seed % 3, 2),
    RR: make(18 + seed % 5, 3),
    TEMP: make(37 + (seed % 4) / 10, 1),
    BP_SYS: make(116 + seed % 18, 8),
    BP_DIA: make(72 + seed % 10, 5),
    PEEP: make(6, 1),
    VT: make(420, 20),
    ETCO2: make(35, 4)
  };
...
function calculateRisk(patient, v){
  const latest = key => (v[key] || [{v:0}]).at(-1).v;
...
async function api(req, url){
  ...
  if(path === '/api/login' && method === 'POST'){
    ...
    const t = token('staff', user.username);
    sessions.set(t, user.username);
  ...
  if(path === '/api/patient/login' && method === 'POST'){
    ...
    const t = token('patient', patient.id);
    patientSessions.set(t, patient.id);
    return json({ok:true, token:t, patient:clone(patient)});
  }
...
export default {
  async fetch(req, env){
    ...
    if(!/\.[a-zA-Z0-9]+$/.test(url.pathname)) url.pathname += '.html';
