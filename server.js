...
function readDb(){ try { return JSON.parse(fs.readFileSync(DATA_PATH, 'utf8')); } catch(e){ return {users:[],patients:[],notes:[],orders:[],labs:[],imaging:[],alerts:[],services:[],audit:[],deviceReadings:[]}; } }
...
function calculateRisk(patient, vitals){ const latest={}; for(const [k,arr] of Object.entries(vitals)){ latest[k]=arr[arr.length-1].v; } const device=latestDeviceValues(patient); if(device&&device.values){Object.assign(latest,device.values);} let score=0; if(latest.HR>110||latest.HR<50)score+=2; if(latest.SPO2<92)score+=3; if(latest.RR>24||latest.RR<8)score+=2; if(latest.TEMP>38.5||latest.TEMP<35)score+=1; if(latest.BP_SYS<90||latest.BP_SYS>160)score+=2; if(patient.status==='critical')score+=3; if(patient.tag==='High Risk')score+=2; const level=score>=7?'high':score>=4?'medium':'low'; return {score,level,latest}; }
function localAiAnalysis(patient,vitals){ const risk=calculateRisk(patient,vitals); const latest=risk.latest; const recommendations=[]; if(latest.SPO2<94)recommendations.push('Review oxygenation, probe position, oxygen delivery, and ventilator settings if applicable.'); if(latest.HR>105)recommendations.push('Review pain, fever, anxiety, hypovolemia, or rhythm issues.'); if(latest.BP_SYS<100)recommendations.push('Review perfusion, fluid balance, and vasopressor needs according to clinician judgment.'); if(patient.tag==='Isolation')recommendations.push('Maintain PPE and isolation workflow.'); if(recommendations.length===0)recommendations.push('Continue routine monitoring and reassess trends.'); return {mode:'mock',generatedAt:now(),patient:{name:patient.name,mrn:patient.mrn,unit:patient.unit,bed:patient.bed},risk,summary:`${patient.name} is currently classified as ${risk.level.toUpperCase()} risk based on simulated vitals, status, and tags.`,recommendations,abnormalValues:Object.entries(latest).filter(([k,v])=>Number(v)>0 && ((k==='SPO2'&&v<94)||(k==='HR'&&(v>105||v<50))||(k==='RR'&&(v>24||v<8))||(k==='TEMP'&&(v>38.5||v<35))||(k==='BP_SYS'&&(v<100||v>160)))).map(([metric,value])=>({metric,value})),disclaimer:'Clinical support preview. Requires clinician review before operational use.'}; }
...
// Device ingestion architecture: medical devices / automated machines -> authorized gateway/API -> LifeView backend -> demo JSON database -> dashboards/mobile -> decision-support analysis.
// Real hospital integrations must be authorized, secure, validated, and implemented through vendor-supported interfaces, HL7/FHIR, gateway APIs, or approved protocols. Do not connect unapproved devices or use real patient data in this public demo.
function requireDeviceKey(req,res,next){
  const configured=process.env.DEVICE_INGEST_API_KEY;
  if(configured && configured!=='change-me-for-demo' && req.headers['x-device-api-key']!==configured) return res.status(401).json({ok:false,error:'Unauthorized device gateway'});
  next();
}
function normalizeDeviceReading(payload){
  const b=payload||{};
  return {id:'dev-'+Date.now()+'-'+Math.random().toString(16).slice(2),ts:normalize(b.ts)||now(),patientId:normalize(b.patientId),mrn:normalize(b.mrn),deviceType:normalize(b.deviceType)||'bedside-monitor',deviceId:normalize(b.deviceId)||'demo-gateway',source:normalize(b.source)||'simulated-device-gateway',values:b.values||{},alerts:b.alerts||[],raw:b};
}
function latestDeviceValues(patient){
  const db=readDb();
  const readings=(db.deviceReadings||[]).filter(r=>(r.patientId&&r.patientId===patient.id)||(r.mrn&&r.mrn===patient.mrn)).sort((a,b)=>String(b.ts).localeCompare(String(a.ts)));
  return readings[0]||null;
}
app.post('/api/device/ingest',requireDeviceKey,(req,res)=>{
  const reading=normalizeDeviceReading(req.body);
  if(!reading.patientId&&!reading.mrn)return res.status(400).json({ok:false,error:'patientId or mrn is required'});
  const db=readDb(); db.deviceReadings=db.deviceReadings||[]; db.deviceReadings.unshift(reading); db.deviceReadings=db.deviceReadings.slice(0,1000);
  const patient=(db.patients||[]).find(p=>p.id===reading.patientId||p.mrn===reading.mrn);
  if(patient && reading.alerts && reading.alerts.length){ db.alerts=db.alerts||[]; reading.alerts.forEach(a=>db.alerts.unshift({id:'a-'+Date.now()+Math.random(),patientId:patient.id,ts:reading.ts,severity:a.severity||'Medium',text:a.text||`${reading.deviceType} alert`})); }
  writeDb(db); audit({username:'device-gateway'},'device_ingest',reading.deviceType,reading.patientId||reading.mrn); res.json({ok:true,reading});
});
app.get('/api/device/latest/:id',requireAuth,(req,res)=>{ const db=readDb(); const patient=(db.patients||[]).find(p=>p.id===req.params.id); if(!patient)return res.status(404).json({ok:false,error:'Patient not found'}); res.json({ok:true,reading:latestDeviceValues(patient)}); });
app.get('/api/device/examples',(req,res)=>res.json({ok:true,examples:{bedsideMonitor:{patientId:'p1',deviceType:'bedside-monitor',deviceId:'MON-ICU-01',values:{HR:118,SPO2:91,BP_SYS:88,BP_DIA:54,RR:28,TEMP:38.6,ETCO2:31},alerts:[{severity:'Critical',text:'Low SpO2 and hypotension threshold crossed'}]},ventilator:{patientId:'p1',deviceType:'ventilator',deviceId:'VENT-07',values:{mode:'AC/VC',FiO2:0.45,PEEP:6,VT:420,RR_SET:18,ETCO2:35}},infusionPump:{patientId:'p1',deviceType:'infusion-pump',deviceId:'PUMP-12',values:{medication:'Demo medication',rateMlHr:12.5,volumeRemainingMl:82,batteryPct:77}},labMachine:{mrn:'MRN-001',deviceType:'lab-analyzer',deviceId:'LAB-CBC-01',values:{test:'CBC',hemoglobin:'11.2 g/dL',wbc:'13.1 10^9/L',flag:'review'}},imagingSystem:{mrn:'MRN-001',deviceType:'imaging-system',deviceId:'PACS-DEMO',values:{modality:'XR',study:'Chest',status:'preliminary',summary:'Demo imaging result awaiting clinician review'}}}}));

app.get('/api/departments',requireAuth,(req,res)=>res.json({ok:true,departments:['ICU','Emergency','Surgery','Radiology','Maternity','Pediatrics','Laboratory','Pharmacy','Biomedical','Administration']}));
...
app.get('/api/vitals/:id',requireAuth,(req,res)=>{ const db=readDb(); const patient=db.patients.find(p=>p.id===req.params.id)||db.patients[0]; const vitals=generateVitals(req.params.id); res.json({ok:true,patient,vitals,risk:calculateRisk(patient,vitals)}); });
app.get('/api/central',requireAuth,(req,res)=>{ const db=readDb(); const unit=req.query.unit; let patients=db.patients||[]; if(unit)patients=patients.filter(p=>p.unit===unit); const cards=patients.map(p=>({patient:p,risk:calculateRisk(p,generateVitals(p.id))})); res.json({ok:true,cards}); });
app.get('/api/ai/analyze/:id',requireAuth,async(req,res)=>{ const db=readDb(); const patient=db.patients.find(p=>p.id===req.params.id); if(!patient)return res.status(404).json({ok:false,error:'Patient not found'}); const vitals=generateVitals(patient.id); const result=process.env.AI_MODE==='openai'?await externalAiAnalysis(patient,vitals):localAiAnalysis(patient,vitals); audit(req.user,'ai_analyze','patient',patient.mrn); res.json({ok:true,analysis:result}); });
...
app.listen(PORT,()=>console.log(`LifeView Central running at http://localhost:${PORT}`));
