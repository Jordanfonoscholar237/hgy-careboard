...
async function sendVerificationEmail(user, code){
  ...
  await transporter.sendMail({from:process.env.MAIL_FROM||process.env.SMTP_USER,to:user.email,subject:'Your HGY CareBoard verification code',text:`Hello ${user.fullName},\n\nYour HGY CareBoard verification code is: ${code}\n\nThis code expires in 10 minutes.\n\nYAOUNDE GENERAL HOSPITAL`,html:`<h2>HGY CareBoard</h2><p>Your code is:</p><div style="font-size:30px;font-weight:bold;letter-spacing:4px">${code}</div><p>YAOUNDE GENERAL HOSPITAL</p>`});
  return {dev:false};
}
...
app.listen(PORT,()=>console.log(`HGY CareBoard Pro Suite v3 running at http://localhost:${PORT}`));
